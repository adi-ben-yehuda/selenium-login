	...
	public static boolean sendMail(int total, int passed, int failed, int skipped) {
		...
		if (sendMail) {
			try {
				...
				/*
				 * Enter smtp host, port, username and password in smtpserver details,
				 * If you are running tests behind proxy, uncomment and enter proxy details
				 */
				Mailer mailer = MailerBuilder.withSMTPServer(mailHost, port, username, pwd)
						.withProperty("mail.smtp.sendpartial", "true").withProperty(
								"mail.smtp.auth", "true")
						.withProperty("mail.smtp.starttls.enable", "true")
						.withTransportStrategy(TransportStrategy.SMTP_TLS).buildMailer();

				Email email = EmailBuilder.startingBlank().from("Automation Execution",
						from).withRecipients(recipients)
						.withSubject(mailSubject + " | " + new SimpleDateFormat(
